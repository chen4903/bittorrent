...
const (
	//上传
	MsgChoke   MsgId = 0 //不支持上传
	MsgUnchoke MsgId = 1 //支持上传
	//下载
	MsgInterested  MsgId = 2 //想要下载
	MsgNotInterest MsgId = 3 //不想要下载
	//数据情况
	MsgHave     MsgId = 4 //告诉甲我现在获得了他想要的哪个piece
	MsgBitfield MsgId = 5
	//数据传输
	MsgRequest MsgId = 6 //下载请求：指定下载的piece，哪里开始，下载多少，长度
	MsgPiece   MsgId = 7 //对方发送的piece的具体数据
	MsgCancel  MsgId = 8 //取消下载
)
...
func handshake(conn net.Conn, infoSHA [SHALEN]byte, peerId [IDLEN]byte) error {
	conn.SetDeadline(time.Now().Add(3 * time.Second)) //超时响应时间
	defer conn.SetDeadline(time.Time{})
	// send HandshakeMsg
	req := NewHandShakeMsg(infoSHA, peerId) //根据infoSHA, peerId来生成握手消息
	_, err := WriteHandShake(conn, req)     //将req写进conn
	if err != nil {
		fmt.Println("send handshake failed")
		return err
	}
	// read HandshakeMsg读出对方给我们的握手消息
	res, err := ReadHandshake(conn)
	if err != nil {
		fmt.Println("read handshake failed")
		return err
	}
	// check HandshakeMsg看看和我们想要的文件是不是一样的
